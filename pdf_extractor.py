...
def extract_tables_from_pdf(
    ...
    # Phase 1: Extract tables per page (from split page files)
    log("[Phase 1] Mengekstrak tabel dari setiap halaman...")
    page_tables: List[Tuple[int, List[List[Any]]]] = []
    ...
    # Only merge when header matches AND previous table ended on consecutive page
    ...
    for page_num, table in page_tables:
        ...
        for mt in merged_tables:
            # Same header AND previous table ended on page_num - 1 (consecutive)
            if _headers_match(mt["header"], header) and mt["end_page"] == page_num - 1:
                mt["rows"].extend(data_rows)
...
def _write_merged_table(
    ...
    """Write a merged table to CSV."""
    header = table_data["header"]
    rows = table_data["rows"]

    out_header = [_normalize_cell(c) or f"col_{i}" for i, c in enumerate(header)]

    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(out_header)
        for row in rows:
            out_row = [_normalize_cell(c) for c in row]
